...
進度（2026-07-15）：WP0、WP0.5、WP1、WP2、WP3、WP4、WP5 已完成並通過交叉審查；G0 已將完整 reviewed stack 收斂到共同基線 `codex/graph-atlas-integration`。`master` 仍停在 `1b035ae`，沒有 production route cutover 或外部部署。

進度補記（2026-07-16）：RG-A 曾於 07-15 完成「`/` → Atlas」route active（commit `f58ddf5`，未部署），07-16 凜空裁決 **Atlas＝第二頁轉接頁、首頁維持 legacy landing**，該 cutover 已在工作樹回退；RG-A 重定義見 Stage A 節。WP5.5 已完成、WP6（Main Galaxy v2 internal）進行中。
...
### Stage A／RG-A — Atlas route release（2026-07-16 重定義為第二頁）

狀態：superseded／已回退（2026-07-16 凜空裁決）。2026-07-15 曾完成「`/` → Atlas」的 route active（commit `f58ddf5`，未部署）；07-16 裁決 Atlas 改為**第二頁轉接頁**、首頁維持 legacy landing，`/` cutover 已在工作樹回退（見 `docs/tasks/2026-07-16-atlas-route-demote-second-page.md`）。RG-A 中與路由無關的產出保留：production 文案（Featured regions）、三語 Wonder title map＋`wonder-titles` 對帳測試、`data-wonder-id` 標記。

重新定義後的 RG-A（未開工，需新 brief 與 Riku 授權）：

- 依賴 G0 integrated＋WP4 branch complete，不依賴 WP6–WP9。
- 將 Atlas 體驗 harden 成正式**第二頁** route `/atlas.html`（2026-07-16 定案）；入口動線定案方案 A：landing「OPEN THE GRAPH」CTA＋footer「Graph」→ `/atlas.html`，搜尋／pills 仍直達 `app.html?node=<id>`；Main／Wonder destinations 維持現行 `app.html`／`wonders.html`。release brief：`docs/tasks/2026-07-16-graph-atlas-rg-a-second-page-release.md`（未授權開工）。
- Atlas 視覺依 2026-07-16 裁決保留白模，本 stage 不含 renderer 升級。
- 19 Wonders 都有可見普通連結；Canvas 仍只呈現核可 previews，不載完整 graph。
- 第二頁 metadata、locale、no-JS、keyboard、mobile、reduced-motion、SEO／discoverability 驗證完成；首頁 landing head contracts 不受影響。
- `/atlas-v2.html` internal route 保留一個 release window。
...
1. RG-A（07-16 重定義）：Atlas 上線為第二頁 route＋首頁 landing 入口動線，目的頁維持 legacy；`index.html` 本身不切換。

...
> **2026-07-15 RG-A／WP5.5：**RG-A 已在 `codex/graph-atlas-integration` commit `f58ddf5` 完成，尚未 push／deploy；WP5.5 的 687-node celestial lock、canonical scene builder、stale-input gate 與分類稽核已完成，尚未開 WP6。現行 gate：Atlas 51／51、Renderer v2 15／15、Depth 4／4、E2E 86／86、來源驗證（687 nodes／3,138 active pairs）、layout 19／0 overlap 全綠；WP5.5 與 WP6 將合併進行跨模型審查。完整順序與狀態語意以 `docs/GRAPH-ATLAS-IMPLEMENTATION-ROADMAP.md` v0.2 為準。
> **2026-07-16 Atlas 定位修訂：**凜空裁決 Atlas＝第二頁轉接頁、首頁維持 legacy landing。RG-A 的 `/` cutover 已在工作樹回退（`index.html` 與 smoke／discoverability／api-failure／visual-styles specs 還原至 `fc8c057` 版；atlas-v2 文案修正、三語 Wonder title map 與 `wonder-titles` 對帳測試保留）。Atlas 暫僅 internal `/atlas-v2.html`；正式第二頁路由需獨立 release brief。見 `docs/tasks/2026-07-16-atlas-route-demote-second-page.md`。
...
- [~] **P1-0 骨架（程式面完成 2026-07-06；剩 `[人工]` D1/KV 綁定）**
  ...
  - [x] `functions/schema.sql` 落地（P1-0 echo / tour_finish；P1-3 追加 event_count）。
  - [x] 「block `/api/*` 四頁 smoke」守門已存在（`tests/e2e/api-failure.spec.ts`，P0 補）；e2e 43 綠、`npm run verify` 全過。
  - [ ] `[人工]` Cloudflare dashboard 建 D1（資料庫名 `neblux-echo`、Pages 綁定名 `DB`）與 KV（綁定名 `LINKS`），並套用 `functions/schema.sql`（`npx wrangler d1 execute neblux-echo --remote --file=functions/schema.sql`）。目前核心 schema：
    ```sql
    ...
    CREATE TABLE IF NOT EXISTS event_count (event TEXT NOT NULL, tour TEXT NOT NULL DEFAULT '', lang TEXT NOT NULL DEFAULT '', step INTEGER NOT NULL DEFAULT 0, is_return INTEGER NOT NULL DEFAULT 0, count INTEGER NOT NULL DEFAULT 0, PRIMARY KEY (event, tour, lang, step, is_return));
    ```
...
- [x] **P1-2 級1 迴響（線上啟用完成 2026-07-08）** — deploy `1c2f647` 上線；線上 curl 驗證 GET/POST/D1 UPSERT 全過（`POST {light,2}`→count 遞增、GET 回 `{"2":1}`）；wonders bundle 含 `/api/echo`（`VITE_ECHO_ENABLED=true` 生效）；landing/explorer 續零 `/api`。
  ...
  - [x] D1 `DB`（`neblux-echo`）已建＋套 P1-2 當時 `functions/schema.sql`（echo / tour_finish）＋綁定 nodus Pages 專案 Production（2026-07-07）。P1-3 後需再套最新版 schema 追加 `event_count`。KV `THROTTLE` 選填、暫未建（缺了只是不節流）。
...
- [~] **P1-3 級3 遙測（程式面完成 2026-07-08；待 `[人工]` prod 啟用）**
  - [x] 後端：`functions/api/event.ts` 寫入 D1 `event_count` 匿名聚合表；不是 Analytics Engine，沒有逐筆事件 log、timestamp、session id、IP 或個資。
  - [x] 前端：`wonders-main.js` 以 `sendBeacon` 打點 `picker_view` / `start` / `step` / `finish` / `drop`；受 `VITE_TELEMETRY_ENABLED` gate，本機/e2e 預設 false。
  - [x] 查詢範本：`docs/telemetry-queries.md` 提供完走率、hook 轉換率、回墜率等 D1 SQL（使用 `neblux-echo --remote`）。
  - [ ] `[人工]` 將最新版 `functions/schema.sql` 套到 prod D1 `neblux-echo`（新增 `event_count`），並於 Cloudflare Pages Production 設 `VITE_TELEMETRY_ENABLED=true` 後重建置。
  - [ ] prod 啟用後，用瀏覽器 Network / D1 查詢驗證五種 beacon 路徑；目前自動測試只守門 dormant path，見 `docs/decisions-ledger.md`。
  - 驗收：`npm run verify` 通過；程式 commit `78c5b5c` + `494ec6f`。線上 active path 待 `[人工]` env/schema 完成後驗證。
  - 未做（債）：無 rate limit；目前數字只可作內部趨勢參考。若要對外發布指標或提高濫用誘因，先補 Cloudflare rate limiting / throttle。

## Phase 2 — 教育（依賴 Phase 0）

- [x] **P2-1 旅程紀錄產物（機制＋19 趟 reflect authoring 完成 2026-07-08）**
  ...
  - [x] 機制：記錄頁 + 星座 + hooks + reflect(fallback outward) + print CSS + e2e 守門；三語瀏覽器實看＋print-media 截圖過；對抗審查 approve、4 findings 已修。
  - [x] `reflect` 各趟內容 authoring：19 趟 tour 三語各 2 題，commit `7d91d37`；`npm run verify` 57 綠。

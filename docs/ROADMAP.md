...
> **2026-07-14 工程基線：**Graph Atlas WP0／WP0.5／WP1／WP2 已完成；現行 production entries 仍使用 legacy runtime 與 `frontend/src/engine/`，WP2 locks 尚未接 Vite/runtime。下一個已授權工作包為 WP3 Bundles & Indices，先做 standalone contracts／generators／artifact audit，不接 production、不進 WP4 UI。基線 branch `codex/graph-atlas-wp3` 由 `8db0092` 建立；原 7-commit checkpoint 同時含 Atlas、Depth、Quant，並非純 WP0–WP2 歷史。

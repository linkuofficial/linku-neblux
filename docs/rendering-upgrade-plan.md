...
> `window.__nebluxApp` / `window.__nebluxExplorer` hook + 真實滑鼠 + 像素取樣。
...
>    focus-glow）改成 app 那套引擎狀態 hook（仿 `window.__nebluxApp`）+ 像素取樣。
...
6. **測試重寫**：E2E 改為（a）暴露 `window.__neblux = { hitTest, getState }` 供 drag/click 斷言；

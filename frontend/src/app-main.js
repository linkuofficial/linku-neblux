...
// Domain stems that mark a cross-domain "bridge" sentence in the English copy
// (e.g. "In biology, ...", "In medical education, ..."). Word-boundary anchored
// stems catch morphological variants (medicine/medical, biology/biological).
// Used to split the long English descriptions into a visible definition +
// collapsible sections. Short or unstructured copy (zh/ja, or any text without
// these markers) yields no bridges and renders as a single <p>.
...
function descSplitSentences(text) {
    // Split on sentence-final punctuation (optionally followed by a closing
    // quote/bracket) + whitespace + a capital or open-paren.
...
function descSectioned(raw) {
    ...
    // Group bridge sentences by domain; any trailing non-bridge sentence is
    // appended to the current group so nothing is dropped.
    const groups = [];
...
// --- Structured sections (en-only enrichment, loaded from /data/sections.json) ---
// Each entry: { lead, core?, impact?, links:[{d:DOMAIN_CODE, t:text}] }. Section
// titles are type-aware (concept/field vs person vs event). When a node has no
// structured sections we fall back to the heuristic descSectioned() on flat text.
const DOMAIN_NAME = {
...
function structLabel(kind, type) {
    ...
    const L = {
        core: {
            en: person ? 'Key contributions' : event ? 'What led to it' : 'The core idea',
            zh: person ? '主要貢獻' : event ? '背景與起因' : '核心概念',
            ja: person ? '主な業績' : event ? '背景と原因' : '核心概念',
        },
        impact: {
            en: person ? 'Legacy' : event ? 'Aftermath' : 'What it changed',
...
function buildGraph() {
    ...
    // is tabindex=0 (keyboard a11y + "Skip to graph" skip-link target), so a
    // mousedown on it focuses it by default. When the top chrome is mid-collapse
    // the search box is briefly pointer-events:none, so a click aimed at it falls
    // through to this full-screen canvas and the focus lands here — swallowing the
    // query the visitor then types. Capture phase runs before d3-zoom's
    // stopImmediatePropagation; preventDefault suppresses focus-on-press only and
    // leaves d3 pan/drag (own window listeners) and node clicks (click event) intact.

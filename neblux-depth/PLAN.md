# Neblux 深度層擴建 — 計畫 v0.2

狀態：in-progress  
更新：2026-07-09  
本版原因：v0.1 經紅隊審查後修正。核心方向保留，但不再用平行 `nodes.json` 當資料源，也不再要求 deployable 頁面使用 executable inline script。
...
`depth_manifest.json` 只記深度頁自己的狀態：
...
- 是否命名為「實驗室」仍未定。M3 前再決定，不影響 M0/M1。
